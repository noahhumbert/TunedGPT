...
def chat_screen():
    ...
    if not 'logged_in' in session or not session['user_email']:
    ...
    if request.method == "POST":
        ...
        if not user_message:
            ...
            return render_template("chat.html", chat_history=chat_history, email=session.get("user_email"), session=session)
